...
main = do
  ...
  if not . null $ compareByLinesFinalFile then do
    ...
    if helpMessage then do 
      ...
      helpPrint helpArg
    else generalF power10 ldc compards html filtering dcfile selStr (prestr, poststr) lineNmb fileDu numTest hc grpp sylD hashStep emptyline splitting (filesave, codesave) concurrently (unwords arg3s) variants1 >> return ()
  ...
  | otherwise = helpFE xs
    where helpFE xs = mapM helpPrint js >> return ()
          js
            ...
            | xs == "+m2" = ["0","5","m2"] 
            | xs == "+m3" = ["0","6","m3"] 
            | otherwise = ["0","1","OR","2","OR","3","OR","4","OR","5","OR","6","OR","7","OR","8","l","w","s","d","p","f","dc","a","b","P","l2","q","ul","r","c","t","C","k","x","m","m2","cm"]
