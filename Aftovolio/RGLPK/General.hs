...
pairs' y@(LL xs n) x
 ...
 | otherwise = error "Aftovolio.RGLPK.General.pairs': Not defined for the arguments. "
...
objLine lng xs arr
 | numElements arr >= lng = mconcat ["obj1 <- c(", intercalate ", " . map (\t -> showFFloat Nothing t "") . objCoeffsNew lng xs $ arr,
      ")", newLineEnding]
 | otherwise = error "Aftovolio.RGLPK.General.objLine: Not defined for the short argument. "
...
minDurations lng mn1 mnSpecial mnG xs1 sps1 = map h [0..lng - 1]
  where xs2
         ...
         | otherwise = error "Aftovolio.RGLPK.General.objLine: Not defined for these arguments. "
        sps2
